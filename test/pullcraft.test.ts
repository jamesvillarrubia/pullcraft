...
describe('PullCraft', () => {
    ...
    describe('getNewFiles', () => {
        it('should get new files', async () => {
            const gitStub = sinon.stub(pullCraft.git, 'diff').resolves('diff --git a/file.txt b/file.txt\n');
            const result = await pullCraft.getNewFiles('develop', 'feature-branch');
            expect(gitStub.calledOnceWith(['--diff-filter=A', 'develop', 'feature-branch', '--', '.', ...pullCraft.exclusions])).to.be.true;
        ...
        it('should handle error when getting new files', async () => {
            ...
            const gitStub = sinon.stub(pullCraft.git, 'diff').rejects(new Error(errorMessage));
            // const consoleErrorStub = sinon.stub(console, 'error');
    ...
    describe('getDiff', () => {
        it('should get diff between branches', async () => {
            ...
            const gitStub = sinon.stub(pullCraft.git, 'diff').resolves(diffOutput);
    ...
    describe('getFilenames', () => {
        it('should get filenames between branches', async () => {
            ...
            const gitStub = sinon.stub(pullCraft.git, 'diff').resolves(filenamesOutput);
