...
describe('PullCraft', () => {
  ...
  describe('Constructor', () => {
    it('should throw an error if OPENAI_API_KEY is not set', () => {
      expect(
        () => new PullCraft({ githubToken: 'fake-github-token' })
      ).to.throw('Error: OPENAI_API_KEY is not set');
    });
    ...
    it('should throw an error if GITHUB_TOKEN is not set', () => {
      expect(
        () =>
          new PullCraft({
            openai: { apiKey: 'fake-openai-api-key' },
            githubStrategy: 'octokit'
          })
      ).to.throw('Error: GITHUB_TOKEN is not set');
    });
  ...
  describe('replacePlaceholders', () => {
    ...
    it('should handle placeholders with different patterns', () => {
      ...
      const result = pullCraft.replacePlaceholders(
        template,
        replacements,
        '{{KEY}}'
      );
  ...
  describe('openUrl', () => {
    ...
    it('should throw an error when URL is not provided', async () => {
      await expect(pullCraft.openUrl('')).to.be.rejectedWith(
        'Error: URL is required'
      );
      expect(
        consoleErrorStub.calledWith(
          'Error: Please provide a value for the argument.'
        )
      ).to.equal(true);
    });
    ...
    it('should open URL on Linux', async () => {
      ...
      expect(execStub.calledWith('xdg-open "https://example.com"')).to.equal(
        true
      );
      expect(
        consoleLogStub.calledWith('Opening URL: https://example.com on linux')
      ).to.equal(true);
    });
    ...
    it('should open URL on macOS', async () => {
      ...
      expect(
        consoleLogStub.calledWith('Opening URL: https://example.com on darwin')
      ).to.equal(true);
    });
    ...
    it('should open URL on Windows', async () => {
      ...
      expect(
        consoleLogStub.calledWith('Opening URL: https://example.com on win32')
      ).to.equal(true);
    });
    ...
    it('should handle error when opening URL', async () => {
      ...
      expect(consoleErrorStub.args[0][0]).to.equal(
        'Error opening URL: Failed to open URL'
      );
    });
  });
  ...
  describe('getRepoInfo', () => {
    it('should get repository info with ssh', async () => {
      sinon
        .stub(pullCraft.git, 'raw')
        .resolves('git@github.com:owner/repo.git\n');
    ...
    it('should get repository info with https', async () => {
      sinon
        .stub(pullCraft.git, 'raw')
        .resolves('https://github.com/owner/repo\n');
      const repoInfo = await pullCraft.getRepoInfo();
      expect(repoInfo).to.deep.equal({ owner: 'owner', repo: 'repo' });
    });
    ...
    it('should throw an error for invalid repo URL', async () => {
      ...
      await expect(pullCraft.getRepoInfo()).to.be.rejectedWith(
        'Failed to get repo info from invalid-url'
      );
    });
  });
  ...
  describe('getNewFiles', () => {
    it('should get new files', async () => {
      ...
      gitStub
        .onSecondCall()
        .resolves('diff --git a/file1.txt b/file1.txt\n...');
      ...
      expect(gitStub.getCall(0).args[0]).to.deep.equal([
        'diff',
        '--name-only',
        '--diff-filter=A',
        'develop',
      ...
      expect(gitStub.getCall(1).args[0]).to.deep.equal([
        'diff',
        'develop',
        'feature-branch',
        '--',
      ...
      expect(gitStub.getCall(2).args[0]).to.deep.equal([
        'diff',
        'develop',
        'feature-branch',
        '--',
    ...
    it('should handle large files', async () => {
      ...
      expect(result).to.include(
        'File large_file.txt is too large to display in the diff. Skipping.'
      );
    });
    ...
    it('should handle error when getting new files', async () => {
      ...
      await expect(
        pullCraft.getNewFiles('develop', 'feature-branch')
  ...
  describe('getModifiedFiles', () => {
    it('should get diff for modified files between branches', async () => {
      ...
      const result = await pullCraft.getModifiedFiles(
        baseBranch,
        compareBranch
      );
    ...
    it('should handle errors', async () => {
      ...
      await expect(
        pullCraft.getModifiedFiles(baseBranch, compareBranch)
      ).to.be.rejectedWith(errorMessage);
      expect(consoleErrorStub.calledOnce).to.equal(true);
    });
    it('should handle large files', async () => {
      ...
      expect(result).to.include(
        'File file1 is too large to display in the diff'
      );
  ...
  describe('getFilenames', () => {
    ...
    it('should handle errors', async () => {
      ...
      const gitStub = sinon
        ...
        .rejects(new Error(errorMessage));
      await expect(
        pullCraft.getFilenames(baseBranch, compareBranch)
      ).to.be.rejectedWith(errorMessage);
      expect(consoleErrorStub.calledOnce).to.equal(true);
    });
  });
  ...
  describe('gptCall', () => {
    ...
    it('should include hint in system prompt when provided', async () => {
      openaiStub.resolves({
        choices: [
          { message: { content: '{"title":"Test Title","body":"Test Body"}' } }
        ]
      });
  ...
  describe('differ', () => {
    it('should return response with exit true when there are no differences', async () => {
      ...
      const getDiffStub = sinon
        .stub(pullCraft, 'getModifiedFiles')
        .resolves('');
      const getNewFilesStub = sinon.stub(pullCraft, 'getNewFiles').resolves('');
      const getFilenamesStub = sinon
        .stub(pullCraft, 'getFilenames')
        .resolves('');
    ...
    it('should return GPT response when there are changes', async () => {
      ...
      const getDiffStub = sinon
        .stub(pullCraft, 'getModifiedFiles')
        .resolves('some diff');
      const getNewFilesStub = sinon
        .stub(pullCraft, 'getNewFiles')
        .resolves('newFile.js');
      const getFilenamesStub = sinon
        .stub(pullCraft, 'getFilenames')
        .resolves('file1.js\nfile2.js');
      const buildTextPromptStub = sinon
        .stub(pullCraft, 'buildTextPrompt')
        .returns('prompt');
      const gptCallStub = sinon
        .stub(pullCraft, 'gptCall')
        .resolves('GPT response');
    ...
    it('should handle errors and log them', async () => {
      const gitStub = sinon
        .stub(pullCraft.git, 'revparse')
        .throws(new Error('revparse error'));

      const result = await pullCraft.differ('develop');
      expect(result).to.equal(undefined);
      expect(
        consoleErrorStub.calledWith('Error generating PR body: revparse error')
      ).to.equal(true);
    });
    ...
    it('should include hint in GPT response when provided', async () => {
      ...
      const buildTextPromptStub = sinon
        .stub(pullCraft, 'buildTextPrompt')
        .returns('prompt with hint');
      const gptCallStub = sinon
        .stub(pullCraft, 'gptCall')
        .resolves('GPT response with hint');
    ...
    it('should return early if dumpTo is set', async () => {
      ...
      const getModifiedFilesStub = sinon
        .stub(pullCraft, 'getModifiedFiles')
        .resolves('modified content');
      const getNewFilesStub = sinon
        .stub(pullCraft, 'getNewFiles')
        .resolves('new content');
      const getFilenamesStub = sinon
        .stub(pullCraft, 'getFilenames')
        .resolves('modified-file.txt\nnew-file.txt');
      const dumpStub = sinon.stub(pullCraft, 'dump');
      ...
      expect(result).to.deep.equal({
        response: 'Diff dumped to dump.txt',
        exit: true
      });
      expect(dumpStub.calledOnce).to.equal(true);
  ...
  describe('createPr', () => {
    it('should create or update a pull request', async () => {
      ...
      const differStub = sinon
        ...
        .resolves({ response: content, exit: false });
      ...
      expect(differStub.calledOnceWith('develop', 'feature-branch')).to.equal(
        true
      );
      expect(getRepoInfoStub.calledOnce).to.equal(true);
      expect(
        listPullsStub.calledOnceWith({
          owner: 'owner',
          repo: 'repo',
          head: 'feature-branch',
          base: 'develop'
    ...
    it('should handle error when creating or updating a pull request', async () => {
      const gitStub = sinon
        .stub(pullCraft.git, 'revparse')
        .rejects(new Error('Failed to get current branch'));
      const differStub = sinon
        .stub(pullCraft, 'differ')
        .resolves({ response: 'PR body', exit: false });
      const getRepoInfoStub = sinon
        .stub(pullCraft, 'getRepoInfo')
        .resolves({ owner: 'owner', repo: 'repo' });
      listPullsStub.resolves([]);
      ...
      expect(
        consoleErrorStub.calledOnceWith(
          'Error creating PR: Failed to get current branch'
        )
      ).to.equal(true);
    });
    ...
    it('should handle error when getting repository info', async () => {
      ...
      const getRepoInfoStub = sinon
        .stub(pullCraft, 'getRepoInfo')
        .rejects(new Error('Failed to get repo info'));

      await pullCraft.createPr('develop');

      expect(consoleErrorStub.calledOnce).to.equal(true);
      expect(consoleErrorStub.firstCall.args[0]).to.include(
        'Error creating PR: Failed to get repo info'
      );
    });
    ...
    it('should handle error when parsing AI response', async () => {
      ...
      const differStub = sinon
        .stub(pullCraft, 'differ')
        .resolves({ response: 'Invalid JSON', exit: false });
    ...
    it('should handle missing body in AI response', async () => {
      ...
      const differStub = sinon
        .stub(pullCraft, 'differ')
        .resolves({
          response: JSON.stringify({ title: 'PR Title' }),
          exit: false
        });
    ...
    it('should handle missing title in AI response', async () => {
      ...
      const differStub = sinon
        .stub(pullCraft, 'differ')
        .resolves({
          response: JSON.stringify({ body: 'PR Body' }),
          exit: false
        });
    ...
    it('should handle error when differ returns undefined', async () => {
      ...
      const differStub = sinon
        .stub(pullCraft, 'differ')
        .resolves({ response: undefined, exit: false });

      await pullCraft.createPr('develop');

      expect(consoleErrorStub.calledOnce).to.equal(true);
      expect(consoleErrorStub.firstCall.args[0]).to.include(
        'Error: Response could not be retrieved.'
      );
    });
    ...
    it('should handle case when differ returns exit: true', async () => {
      ...
      sinon
        .stub(pullCraft, 'differ')
        .resolves({ response: 'No changes', exit: true });
    ...
    it('should update an existing PR if one exists', async () => {
      ...
      expect(openUrlStub.firstCall.args[0]).to.equal(
        'https://github.com/owner/repo/pull/123'
      );
      expect(
        consoleLogStub.calledWith('Updating existing PR #123...')
      ).to.equal(true);
    });
  ...
  describe('dump', () => {
    it('should dump diff to a file', () => {
      ...
      expect(
        fsWriteFileSync.calledOnceWith('test-dump.txt', 'Test diff content')
      ).to.equal(true);
  ...
  describe('buildTextPrompt', () => {
    it('should build text prompt with replacements', () => {
      ...
      pullCraft.standardReplacements = {
        baseBranch: 'main',
        compareBranch: 'feature'
      };
      pullCraft.openaiConfig.titleTemplate =
        'PR: __customKey__ from __compareBranch__ to __baseBranch__';
